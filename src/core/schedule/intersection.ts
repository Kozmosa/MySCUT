...
export type IntersectionDisplayMode =
  | "default"
  | "availableOnly"
  | "unavailableOnly";
...
function resolveLessonInterval(
  lesson: ScheduleLesson,
  timeSlotMap: Map<number, TimeInterval>
) {
...
export function buildIntersectionSchedule(
  ...
  unavailableColor = "#d4380d",
  allAvailableColor = "#52c41a"
): ScheduleData {
  ...
  const maxWeek = Math.max(
    1,
    ...participants.map(
      (participant) => participant.scheduleData.table.maxWeek || 1
    )
  );
  ...
  participants.forEach((participant) => {
    ...
    const participantIntervalByNode =
      buildTimeSlotIntervalMap(participantTimeSlots);

    participant.scheduleData.lessons.forEach((lesson) => {
      const lessonInterval = resolveLessonInterval(
        lesson,
        participantIntervalByNode
      );
      if (!lessonInterval) {
        return;
      }
      ...
      for (let week = lesson.startWeek; week <= lesson.endWeek; week += 1) {
        ...
        unionTimeSlots.forEach((unionSlot) => {
          ...
          if (
            !unionInterval ||
            !isIntervalOverlapped(lessonInterval, unionInterval)
          ) {
            return;
          }
  ...
  for (let week = 1; week <= maxWeek; week += 1) {
    for (let day = 1; day <= 7; day += 1) {
      unionTimeSlots.forEach((slot) => {
        const occupied =
          occupiedByCell.get(createCellKey(week, day, slot.node)) ??
          new Set<string>();
        const unavailable = sortNames(Array.from(occupied));
        const available = sortNames(
          allNames.filter((name) => !occupied.has(name))
        );
        const label = buildIntersectionCellLabel(mode, available, unavailable);

        let color = "";
        if (mode === "unavailableOnly") {
          color = unavailable.length > 0 ? unavailableColor : allAvailableColor;
        }

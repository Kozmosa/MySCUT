...
function ScheduleIntersectionPage() {
  ...
  const [isHtmlImportMethodModalOpen, setIsHtmlImportMethodModalOpen] =
    useState(false);
  const [isHtmlInputModalOpen, setIsHtmlInputModalOpen] = useState(false);
  const [htmlImportMethod, setHtmlImportMethod] =
    useState<HtmlImportMethod>("file");
  const [htmlInputText, setHtmlInputText] = useState("");
  const [isLocalScheduleModalOpen, setIsLocalScheduleModalOpen] =
    useState(false);
  ...
  const [displayMode, setDisplayMode] =
    useState<IntersectionDisplayMode>("default");
  ...
  const handleImportCompressedQmsFromClipboardEntry = async () => {
    ...
    try {
      ...
      openExternalNameModal(
        parsedQms.scheduleData,
        "QMS",
        parsedQms.timeSlotPresetId
      );
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "压缩QMS导入失败";
  ...
  const handleConfirmHtmlImportMethod = async () => {
    ...
    try {
      ...
      openExternalNameModal(scheduleData, "华工教务HTML", "builtIn");
    } catch (error) {
  ...
  const openExternalNameModal = (
    scheduleData: ScheduleData,
    source: string,
    timeSlotPresetId: TimeSlotPresetId = "builtIn"
  ) => {
  ...
  const handleConfirmExternalName = () => {
    ...
    const finalName =
      pendingExternalName.trim() || pendingExternalSchedule.name;
  ...
  const handleImportQms = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "QMS 课表导入失败";
      messageApi.error(errorMessage);
    } finally {
      event.target.value = "";
    }
  ...
  const handleCalculate = () => {
    ...
    const participants = [
      ...
      ...externalSchedules.map((item) => ({
        ...
        timeSlotPresetId: item.timeSlotPresetId ?? ("builtIn" as const),
      })),
    ];
          ...
          {savedSchedules.length === 0 ? (
            ...
            savedSchedules.map((schedule) => (
              <button
                key={schedule.id}
                type="button"
                className={`schedule-switch-item ${
                  schedule.id === selectedLocalScheduleId ? "is-active" : ""
                }`}
                onClick={() => {

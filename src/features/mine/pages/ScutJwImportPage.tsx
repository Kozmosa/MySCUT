...
function ScutJwImportPage() {
  ...
  const customUrlError =
    accessMode === "custom" && customUrl.trim() && !entryUrlResult.ok
  ...
  useEffect(() => {
    ...
    Modal.info({
      title: "导入提示",
      content:
        "请选择当前可用的访问方式，登录教务系统并打开“个人课表查询”栏目，然后再点击“开始导入当前页面”。",
      okText: "知道了",
    });
  }, [isAndroidNative]);
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      // Only keep listeners registered while still unmounted on android native
      if (isUnmounted || !Capacitor.isNativePlatform()) {
        return;
      }

      const closedHandle = await InAppBrowser.addListener(
        "browserClosed",
        () => {
          if (isUnmounted) {
            return;
      ...
      const navigationHandle = await InAppBrowser.addListener(
  ...
  const handleOpenBrowser = async () => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "无法打开教务系统页面";
  ...
  const handleStartImport = async () => {
    ...
    try {
      ...
      debugLog(
        "[ScutJwImport] 步骤3/6: HTML 获取成功, 长度:",
        htmlText.length,
        "字符"
      );
      debugLog(
        "[ScutJwImport] HTML 前 200 字符:",
        htmlText.substring(0, 200).replace(/\n/g, "\\n")
      );
      debugLog(
        "[ScutJwImport] HTML 后 200 字符:",
        htmlText
          .substring(Math.max(0, htmlText.length - 200))
          .replace(/\n/g, "\\n")
      );

      const fallbackSemesterStartDate = getSemesterStartDate();
      debugLog(
        "[ScutJwImport] fallbackSemesterStartDate:",
      ...
      debugLog(
        "[ScutJwImport]   - 课程列表:",
        scheduleData.courses.map((c) => c.name).join(", ")
      );
      ...
      scheduleData.lessons.forEach((lesson, i) => {
        debugLog(
          `[ScutJwImport]   lesson[${i}]: day=${lesson.day} node=${lesson.startNode}-${lesson.endNode} week=${lesson.startWeek}-${lesson.endWeek} step=${lesson.weekStep} room=${lesson.room} teacher=${lesson.teacher}`
        );
      });

      const themePreset = resolveScheduleImportThemePreset(
        getScheduleThemeId()
      );
      const themeId = themePreset.id;
      const nextSemesterStartDate =
        scheduleData.table.startDate || fallbackSemesterStartDate;
      ...
      const errorMessage =
        error instanceof Error ? error.message : "华工教务课表导入失败";
  ...
  return (
    <section className="schedule-settings-page">
      ...
      <div className="schedule-settings-content">
        {!isAndroidNative ? (
          <p className="schedule-pdf-error">
          ...
          <>
            <fieldset className="scut-jw-access-group">
              ...
              <div className="scut-jw-access-options">
                {SCUT_JW_ACCESS_OPTIONS.map((option) => (
                  <label
                    key={option.value}
                    className={`scut-jw-access-option ${
                      accessMode === option.value ? "is-active" : ""
                    }`}
                  >
                    <input
                      type="radio"
                    ...
                    <span className="scut-jw-access-copy">
                      ...
                      <span className="scut-jw-access-description">
                        {option.description}
                      </span>
                    </span>
                  </label>
              ...
              {accessMode === "custom" ? (
                <div className="scut-jw-custom-url">
                  <Input
                    value={customUrl}
                    onChange={(event) =>
                      handleCustomUrlChange(event.target.value)
                    }
                  ...
                  <p
                    ...
                    className={`scut-jw-custom-url-hint ${
                      customUrlError ? "is-error" : ""
                    }`}
            ...
            <div className="scut-jw-tip-card">
              ...
              <p className="scut-jw-tip-text">
                1. 选择访问方式，点击“打开教务系统”并完成登录
              </p>
              <p className="scut-jw-tip-text">
                2. 在内置浏览器中进入“个人课表查询”栏目
              </p>
              <p className="scut-jw-tip-text">
                3. 返回此页点击“开始导入当前页面”
              </p>
            ...
            <div className="scut-jw-status-card">
              <p className="schedule-settings-current-date">
                当前捕获页面：{lastNavigatedUrl || "尚未检测到导航页面"}
              </p>
              <p className="schedule-settings-current-date">
                浏览器状态：
                {isBrowserClosed
